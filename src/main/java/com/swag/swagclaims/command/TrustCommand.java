...
/** /trust <player> — grants BUILD trust in the claim the sender is standing in. */
public class TrustCommand implements CommandExecutor, TabCompleter {

    private final SwagClaimsPlugin plugin;

    public TrustCommand(SwagClaimsPlugin plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ClaimCommandUtil.handleTrustGrant(plugin, sender, label, args, TrustLevel.BUILD);
        return true;
    }
