...
/** /containertrust <player> — grants CONTAINER trust in the claim the sender is standing in. */
public class ContainerTrustCommand implements CommandExecutor, TabCompleter {

    private final SwagClaimsPlugin plugin;

    public ContainerTrustCommand(SwagClaimsPlugin plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ClaimCommandUtil.handleTrustGrant(plugin, sender, label, args, TrustLevel.CONTAINER);
        return true;
    }
